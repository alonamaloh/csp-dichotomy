...
\item[\textbf{D10}] \textbf{The arity in Lemma~\ref{lem:special-wnu} is
  suspect.} The imported statement gives a special WNU of arity $n^{n!}$; the
  period of $y\mapsto w(x,\dots,x,y)$ need not divide $n^{n!}$ --- take $n=3$
  and an element of order $5$. The remedy is to weaken the statement to
  ``\emph{some} arity $N$'', which is all the dichotomy proof uses. The precise
  arity matters only for \S4 of the source, which is cut.
...
Ten blocking items in a fifty-page paper is not a high rate; it is roughly what
...
hypotheses that are too weak or missing (D5, D6, D8), and three are artefacts of
the interface with the older paper (D7, D9, D10). None of them threatens the
