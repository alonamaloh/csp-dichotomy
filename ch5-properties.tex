...
Let $R\le\mathbf A_{1}\times\dots\times\mathbf A_{m}$ and
$C_{i}\subte{T}A_{i}$ for every $i$, where $T\in\{\TBA,\TC\}$. Then
$\proj_{1}\bigl(R\cap(C_{1}\times\dots\times C_{m})\bigr)\dsubte{T}A_{1}$.
\end{lemma}

\begin{hazard}\label{haz:ba-center-implies}
Lemma~\ref{lem:ba-center-implies} is the workhorse of case (2) of
Theorem~\ref{thm:main-inductive} and is imported from \cite{ZhukStrong}. It is
in the same family as the results already formalized in the predecessor project
(\S\ref{sec:reuse}) --- both concern the behaviour of binary absorption and
centrality under intersection with a box and projection --- but it is
\emph{not} the same statement and is not currently available. It is the single
highest-value import to formalize first, because it is small, self-contained,
and used everywhere.
\end{hazard}
