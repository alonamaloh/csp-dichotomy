...
(\S\ref{sec:hedges}). Nineteen substantive defects in fifty pages is roughly
...
Defects confirmed and \textbf{open} & 3 \\
...
extension; $\mathrm{D}19$ (\S\ref{sec:D19}), the undischarged
leave-one-out hypotheses of the two stable-intersection applications in the
typing argument; and $\mathrm{D}21$ (\S\ref{sec:D21}), the asserted
cross-component step in the linkedness-preservation auxiliary.
$\mathrm{D}17$ was declared solved in an earlier draft; an
...
(Entry~\ref{ent:route-four}). $\mathrm{D}19$'s mixed-type half is since
repaired outright --- the anchored statement it called for is now
\pkey{lem:anchored-intersection}, proved (\S\ref{sec:D19}) --- while its
same-class half, and $\mathrm{D}17$, remain unresolved; the debt has been
moved to statements that are plausibly provable, and the reasons the old
routes failed are now theorems rather than suspicions.
