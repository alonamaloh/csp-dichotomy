...
\item[Open mathematics.] $\mathrm D17$ (\S\ref{sec:D17}); the
  ...
  \texttt{LEMNoBridge\brk Between\brk DifferentTypes}; and the step
  \pkey{cl:multiply-step} inside \pkey{lem:multiply-all-linear}.
  These gate the statements that depend on them. Two items listed here in earlier drafts have since come off: the
  reflexivisation, which needs no argument at all, and the induction measure
  across expanded coverings (Entry~\ref{ent:measure}), which was never in
  difficulty.
