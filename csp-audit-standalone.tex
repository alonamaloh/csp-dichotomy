...
(\S\ref{sec:hedges}). Eighteen substantive defects in fifty pages is roughly
...
\textbf{} & \textbf{Count} \\
\hline
Defects confirmed and repaired, needing no new mathematics & 16 \\
...
\quad omitted steps and dropped hypotheses & 9 \\
Defects confirmed and \textbf{open} & 2 \\
...
The open items are $\mathrm{D}17$ (\S\ref{sec:D17}), the maximal multi-type
extension, and $\mathrm{D}19$ (\S\ref{sec:D19}), the undischarged
leave-one-out hypotheses of the two stable-intersection applications in the
typing argument. $\mathrm{D}17$ was declared solved in an earlier draft; an
external review found the repair invalid at its first line, and the finding is
correct. It is recorded here as open rather than quietly weakened.

Both have since been sharpened, and $\mathrm{D}17$ has been \emph{routed
around}: the proof paper re-proves the parallelogram lemma without
\texttt{LEMMaximalMultExtention}, so the extra hypothesis it once carried is
no longer consumed anywhere, and what remains of both defects is carried by
one claim and one stated lemma of the proof paper
(Entry~\ref{ent:route-four}). Neither is resolved; the debt has been moved to
statements that are plausibly provable, and the reasons the old routes failed
are now theorems rather than suspicions.
\clearpage
%% ---------------------------------------------------------------- audit1-defects.tex
\section{The register}\label{sec:register}

Eighteen places where \cite{ZhukSimplified} is wrong, incomplete, or states
less than its own proof gives. Sixteen are repaired and none of those repairs
needs new mathematics. Two are open: $\mathrm{D}17$ and $\mathrm{D}19$ ---
though neither now blocks the proof paper in its original form, since the
route-four rewrite of Entry~\ref{ent:route-four} moved what remains of both
into one claim and one stated lemma there.
...
$\mathrm D{19}$ & \texttt{main:2663, 2693} & Two stable-intersection
  ...
  $D^{(2)}$ equals $D^{(1)}$. \textbf{Open} &
  \pkey{lem:parallelogram-\brk crucial}, \pkey{cl:typing-anchor} \\
...
Residue: \pkey{cl:typing-anchor} --- the bad branch, the two Case-B corner
configurations, and $n=2$ for $T=\TPC$ --- and \pkey{lem:typed-above} for
...
is not known. What would repair it is an \emph{anchored} form of the
corollary --- $R$ subdirect onto the $B_{j}$ rather than onto the full
domains, which is exactly what $1$-consistency of $D^{(1)}$ supplies ---
and that anchored form is a sibling of \texttt{FitInLinearSubuniverses} of
the JACM proof rather than of the corollary as printed.

\emph{Status: open.} The proof paper does not repair the source's argument;
its new proof of \pkey{lem:parallelogram-crucial} avoids both applications
on the generic branches (Entry~\ref{ent:route-four}) and quarantines what
they were needed for in \pkey{cl:typing-anchor}, where the corner cases
($\TBA$/$\TC$ chain steps, and $n=2$ for $T=\TPC$) still want precisely
such an anchored statement.
...
\emph{Repair.} Restore the hypothesis: the one-constraint instance is
...
Caveat \pkey{haz:parallelogram-hyp}.
...
\begin{itemize}\tightlist
\item \textbf{``Replacing a constraint by all strict weakenings need not make
  progress'' is true and harmless.} The reviewer's \S8 is right that the
  conjunction of all constraints weaker than $R$ can be $R$ itself, so that
  $\mathcal I[C\Uparrow]$ can have the same solution set as $\mathcal I$; and
  right that a measure counting admitted tuples points the wrong way. It does
  \emph{not} follow that the construction fails to make progress, and an
  earlier draft of ours accepted that it did and recast cruciality with a
  single-step quantifier. Progress is measured on the constraints: $C$ is
  replaced by finitely many strictly weaker ones, so the multiset of
  $\nu$-values drops whether or not the solution set moves. The single-step
  reading also turns out to be too weak for
  \pkey{lem:crucial-irreducible}, whose proof imposes two weaker constraints
  \emph{together}. Proof paper: \pkey{def:crucial},
  \pkey{lem:weakening-terminates}, \pkey{haz:weakening}.
