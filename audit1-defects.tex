...
$\mathrm D6$ & \S3.2 & Bridges-from-relations applied without its third
  hypothesis, which can fail & \pkey{lem:crucial-split-free},
  \pkey{lem:connected} \\
...
Proof paper: the property is isolated as \emph{split-freeness}
(\pkey{def:split}); \pkey{lem:crucial-\brk split-free} proves both the equivalence
with the witness configuration and that cruciality forbids splitting; and
\pkey{lem:connected} carries split-freeness of the constraints as a
hypothesis and is now \textsc{proved} with it. Both call sites discharge the
hypothesis through cruciality --- the (1c) subinstance consists of
constraints of a covering crucial in the ambient reduction, and the
codimension-one instance is itself crucial --- so the repair costs the
consumers nothing.
