...
%%  Generated from revision 1d50a54 on 2026-08-02.
%% =====================================================================
...
Replace $\delta$ by $\delta\cap(\cover{\sigma_{1}}\times\cover{\sigma_{2}})$,
which is again a bridge with the same collapse, and assume $\bcol\delta$
rectangular (Caveat~\ref{haz:rectangularise}).
...
$(a,c)\notin\bcol\delta$; choosing $e$ with $(e,c)\in\bcol\delta$ ---
possible because $\proj_{1}(\bcol\delta)$ meets the block of $c$ --- gives
$(e,a,c,d)\in\delta''$ with $(e,a)\notin\sigma_{1}$. Take $\omega=\delta''$.
...
$(a,b)\in\cover{\sigma_{1}}$ with $(a,b,c,d)\in\omega$, and since
$(a,b)\in\cover{\sigma_{1}}\subseteq\LeftLinked(\bcol\omega)$ and
$(a,c)\in\bcol\omega$ we get $(a,a,c,c),(b,b,c,c)\in\omega$. Sending
...
\begin{hazard}[Two normalisations that are asserted]\label{haz:rectangularise}
The proof opens with ``we may assume $\bcol\delta$ is rectangular, as
otherwise compose it with itself many times''. Nothing here establishes that
the stabilised composite is still a bridge with the properties the rest of the
proof uses. Note that this is \emph{not} the same situation as the reflexivity
of Caveat~\ref{haz:reflexivise}, which turned out to be free: rectangularity of
$\bcol\delta$ does not follow from any hypothesis in sight, and a genuine
replacement of $\delta$ is being made. It is used only to make $\proj_{1,3}$
behave, and the case split on $\delta'$ immediately afterwards does much the
same work, so it may well be removable; that has not been checked.

The step ``choosing $e$ with $(e,c)\in\bcol\delta$'' also needs
$\proj_{2}(\bcol\delta)$ to meet the relevant block, which holds after the
restriction to $\cover{\sigma_{1}}\times\cover{\sigma_{2}}$ but is not
argued.

Until both are settled, Lemma~\ref{lem:no-cross-bridge} and everything
downstream of it --- including the $\TPC$ case of
Theorem~\ref{thm:stable-intersection} --- rest on them. See
\S\ref{sec:status}.
\end{hazard}
...
\item[\textsc{open}] Confirmed to need an argument that is not available.
  ...
  written into statements rather than left implicit, and the normalisation of
...
\pkey{lem:linear-equiv}, \pkey{lem:bridge-reflexive} & \textsc{proved} & \\
\pkey{lem:no-cross-bridge}, \pkey{lem:bridge-to-pc} & \textsc{outlined} &
  written out in \S\ref{sec:bridge-proofs}; both rest on the two
  normalisations of Caveat~\ref{haz:rectangularise} \\
...
Four items block the rest, and they are the honest reading of the table.
...
\item The rectangularity normalisation of Caveat~\ref{haz:rectangularise},
  on which \pkey{lem:no-cross-bridge} and \pkey{lem:bridge-to-pc} rest, and
  through them the $\TPC$ case of \pkey{thm:stable-intersection}.
...
\item[] \emph{Two entries stood here and no longer do.} The induction measure
