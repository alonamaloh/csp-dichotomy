...
%%  Generated from revision f3274b2 on 2026-08-02.
%% =====================================================================
...
Lemma~\ref{lem:weakening-terminates} settles the single-instance case, and the
multiset extension of a well-founded order is well founded, so the shape of the
argument is available; but condition 3 replaces a member by \emph{all} weaker
instances, which is the operation Caveat~\ref{haz:weakening} rejects, and it
has to be recast as a single step before that extension can be applied.
(ii) The
...
\item[\textsc{open}] Confirmed to need an argument that is not available.
  ...
  Hypothesis~\ref{hyp:maximal-mult}, which is written into a statement rather
  than left implicit, and the normalisation of
...
Five items block the rest, and they are the honest reading of the table.
...
\item The induction measure across expanded coverings
  (Caveat~\ref{haz:the-induction}(c)). The termination of weakening, listed
  here until now, is \pkey{lem:weakening-terminates}; what remains of it is the
  \emph{set}-valued iteration of Construction~\ref{constr:omega}, whose
  condition 3 still replaces a member by all weaker instances
  (Caveat~\ref{haz:omega}).
\end{enumerate}
