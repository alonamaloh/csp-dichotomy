...
%%  Generated from revision 3016fec on 2026-08-02.
%% =====================================================================
...
$T\in\{\TL,\TPC\}$. Configuration \eqref{eq:config-gamma} does not occur.
And if $T=\TPC$ then $n=2$.
\end{claim}
...
irreducibility (Step~2); the one-class fact (Step~3); and, at each
coordinate, the reduction of the typing to the three displayed
configurations. The claim is the residue, and it is a sibling of
Claim~\ref{cl:multiply-step}: configuration \eqref{eq:config-gamma} is the
same single-type $\TBA$/$\TC$ obstruction as
Caveat~\ref{haz:multiply-citation}(a), and the class part of
\eqref{eq:config-beta} is the same type-$\TD$-step mismatch as
Caveat~\ref{haz:multiply-citation}(b).
...
proof cites --- is acyclic, over $114$ statements, $78$ of which carry proofs,
and $182$ edges. That is a check on the arrangement, not on the mathematics,
...
\pkey{lem:parallelogram-crucial} & \textsc{outlined} & the parallelogram
  property, irreducibility, the one-class fact, and the good branch of the
  typing are proved; what remains is Claim~\ref{cl:typing-anchor} and the
  stated \pkey{lem:typed-above}. Two hypotheses restored: subdirectness of
...
shape: of its sixty-two statements, forty are proved here --- one of them,
